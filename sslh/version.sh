#!/bin/bash
# Get latest upstream SSLH version
# Supports multiple output formats for different use cases:
#   version.sh                 → full version with suffix (e.g., v2.3.0-alpine)
#   version.sh --upstream      → raw upstream version (e.g., v2.3.0)
...
upstream_version=$("$(dirname "$0")/../helpers/latest-git-tag" yrutschle sslh "v.+$")
