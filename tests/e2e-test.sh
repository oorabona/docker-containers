...
test_container() {
    ...
    case "$container" in
        ...
        terraform|opentofu)
            ...
            # Terraform's entrypoint ends in `exec /bin/terraform "$@"` and
            # OpenTofu's ends in `exec /usr/local/bin/tofu "$@"`, so any command
            # passed becomes a CLI argument. Override the entrypoint so the
