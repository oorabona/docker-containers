...
**What is retained.** The image ships every retained TimescaleDB `.so` that successfully builds for the Alpine/musl platform. A persisted database on any such retained version **starts without any migration** — PostgreSQL loads the `.so` matching the catalog version. A version that fails to compile under musl is tolerated and excluded; it is recorded in the versionset artifact (`.build-lineage/ext-timescaledb-pg<major>-versionset.json`, `excluded[]`) and not shipped — a database on such a version would still need the upstream HA image for recovery. The retained window is derived from the upstream `timescale/timescaledb-ha` support matrix, so it differs per PostgreSQL major (a TimescaleDB version that never supported a given PostgreSQL major is not retained for it):
