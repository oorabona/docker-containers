...
while IFS= read -r -d '' j2file; do
    ...
    if [ -d "$outfile" ]; then
        echo "Could not replace Terraform output for template ${j2file}" >&2
        exit 1
    fi
    tmp=$(mktemp "$(dirname -- "$outfile")/.tf-render.XXXXXX") || {
        echo "Could not create temporary file for Terraform template ${j2file}" >&2
        exit 1
    }
    if ! jinja2 "${j2file}" "${CONFIGFILE}" > "$tmp"; then
        echo "Could not render Terraform template ${j2file}" >&2
        exit 1
    fi
    if [ -f "$outfile" ] && [ ! -L "$outfile" ]; then
        mode=$(stat -c %a -- "$outfile") || {
            echo "Could not determine mode for Terraform output of template ${j2file}" >&2
            exit 1
        }
    else
        umask_value=$(umask)
    ...
    if ! chmod "$mode" "$tmp"; then
        echo "Could not set mode for Terraform output of template ${j2file}" >&2
        exit 1
    fi
