...
Upstream releases new version
  ...
  auto-build.yaml
        │
        ▼
  Build digest label          ← source-cache provenance
  recorded on matrix and direct `./make push` builds
        │
        ▼
...
- **Build digest labels** — matrix builds and direct `./make push` builds carry the descriptive source-cache label; Bake records its OCI `containerimage.digest` (`sha256:`) as lineage `build_digest` ([ADR-002](docs/adr/ADR-002-smart-rebuild-detection.md))
